-- Auto post recurring templates into actual transactions.

alter table public.recurring_templates
  add column if not exists auto_post_as_actual boolean not null default false;

alter table public.transactions
  add column if not exists auto_post_template_id uuid references public.recurring_templates (id) on delete set null,
  add column if not exists auto_post_month date;

create unique index if not exists idx_transactions_auto_post_unique
  on public.transactions (household_id, auto_post_template_id, auto_post_month)
  where auto_post_template_id is not null and auto_post_month is not null;

create index if not exists idx_transactions_auto_post_template
  on public.transactions (auto_post_template_id, auto_post_month);
...
begin
  ...
  loop
    ...
    on conflict (household_id, auto_post_template_id, auto_post_month) do nothing;
  end loop;
end;
$$;
